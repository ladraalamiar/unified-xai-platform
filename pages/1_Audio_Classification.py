...
def load_model(model_name):
    ...
    try:
        from models.audio.model_loader import load_audio_model
...
tab_upload, tab_test = st.tabs(["📤 Upload Audio", "🧪 Test Samples"])

audio_file = None
audio_source = None  # Pour tracker la source
...
# TAB 2: Test Audio
with tab_test:
    st.write("**Available test audio files:**")
    
    # AJUSTEZ ces chemins selon vos fichiers
    test_audios = {
        'Test Sample 1': 'models/audio/.pb/wav_de_test/test_audio.wav',
        'Test Sample 2': 'test_data/audio/sample1.wav',
        'Real Voice Example': 'test_data/audio/real.wav',
        'Fake Voice Example': 'test_data/audio/fake.wav',
    }
    ...
    if not available:
        ...
        Add test audio files to:
        - `models/audio/.pb/wav_de_test/`
        - `test_data/audio/`
        ...
        selected = st.selectbox(
            ...
            key="test_audio_selector"
        )
        
        if selected != "-- Select --":
            test_audio_path = available[selected]
            ...
            try:
                ...
                # Ouvrir pour processing
                audio_file = open(test_audio_path, 'rb')
                audio_source = "test"
                ...
                try:
                    ...
                    y_temp, sr_temp = sf.read(test_audio_path)
                    duration_temp = len(y_temp) / sr_temp
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        st.metric("Duration", f"{duration_temp:.2f}s")
                    with col2:
                        st.metric("Sample Rate", f"{sr_temp} Hz")
                except:
                    pass
                
            except Exception as e:
                st.error(f"❌ Error loading test audio: {str(e)}")
                audio_file = None

# Indicateur de source
if audio_file:
    if audio_source == "uploaded":
        st.caption("🔹 Using uploaded audio file")
    elif audio_source == "test":
        st.caption("🔹 Using test audio file")

# Arrêter si pas d'audio
if audio_file is None:
    st.info("👆 **Please upload an audio file or select a test sample to begin**")
    st.stop()

# ============================================
# RESTE DU CODE (FONCTIONNE MAINTENANT AVEC LES DEUX SOURCES)
# ============================================

# Charger et visualiser audio
try:
    import soundfile as sf
        ...
        with st.spinner("Classifying audio..."):
            try:
                ...
                with st.expander("📊 Detailed Results", expanded=False):
                    st.write("**Full Prediction Details:**")
                    ...
                    if 'probabilities' in prediction:
                        ...
                        with prob_col1:
                            real_prob = prediction['probabilities'].get('REAL', 1 - confidence if pred_class == 'FAKE' else confidence)
                            st.metric("🟢 REAL", f"{real_prob:.2%}")
                            st.progress(float(real_prob))
                        
                        with prob_col2:
                            fake_prob = prediction['probabilities'].get('FAKE', confidence if pred_class == 'FAKE' else 1 - confidence)
                            st.metric("🔴 FAKE", f"{fake_prob:.2%}")
                            st.progress(float(fake_prob))
                    
                    # Info modèle
                    st.write(f"**Model Used**: {model.name}")
                    st.write(f"**Audio Source**: {audio_source}")
                
                # Sauvegarder dans session state pour XAI
...
if 'audio_prediction' in st.session_state:
    ...
    with st.expander("ℹ️ XAI Methods for Audio", expanded=False):
        st.markdown("""
        ### Available Methods
        
        **🎨 Gradient Visualization** (~5-10s)
        - Attention heatmap on spectrogram
        - Shows where model "listens"
        - Fast and intuitive
        
        **🟢 LIME** (~30-60s)
        - Time-frequency region importance
        - Model-agnostic
        - Shows which segments matter
        ...
        **📈 Comprehensive** (~5s)
        - All audio features breakdown
        - Waveform, spectrogram, MFCCs
        - Technical deep-dive
        """)
    ...
    with xai_col1:
        xai_method = st.selectbox(
            ...
            help="Choose explainability method"
        )
    ...
    if xai_btn:
        # Map vers fonction
        method_map = {
            "🎨 Gradient Visualization (Fast - Recommended)": "gradient",
            "📊 Comprehensive Analysis (All Features)": "comprehensive",
            "🟢 LIME (Time-Frequency Regions)": "lime",
            "📈 SHAP (Feature Importance)": "shap"
        }
        
        method_key = method_map[xai_method]
        ...
        with st.spinner(f"Generating {xai_method}..."):
            try:
                ...
                if method_key == "gradient":
                    fig = gradient_audio_visualization(model, st.session_state['audio_file'], prediction, sr=sr)
                elif method_key == "comprehensive":
                    fig = comprehensive_audio_analysis(model, st.session_state['audio_file'], prediction, sr=sr)
                elif method_key == "lime":
                    fig = audio_lime_explanation(model, st.session_state['audio_file'], prediction, sr=sr)
                elif method_key == "shap":
                    fig = audio_shap_explanation(model, st.session_state['audio_file'], prediction, sr=sr)
                
                st.pyplot(fig)
                plt.close()
                
                st.success(f"✅ {xai_method} generated successfully!")
                
                # Guide d'interprétation
                with st.expander("💡 How to Interpret This Explanation"):
                    if method_key == "gradient":
                        st.markdown("""
                        ### Gradient Visualization Guide
                        
                        **Red/Yellow areas**: High attention - model focuses here
                        **Blue/Purple areas**: Low attention - less important
                        
                        **For FAKE audio**:
                        - Often shows focus on high frequencies (artifacts)
                        - Unusual patterns in specific time segments
                        
                        **For REAL audio**:
                        - More balanced attention across frequencies
                        ...
                        **7 visualizations** show different aspects:
                        1. Waveform: Time-domain signal
                        2. Mel Spectrogram: Frequency content
                        3. MFCCs: Speech characteristics
                        4. Chromagram: Pitch distribution
                        5-7. Various spectral features
                        
                        Look for:
                        - Unusual patterns in any visualization
                        - Artifacts in high frequencies (FAKE)
                        ...
                        **What it means**:
                        ...
                        - Distributed regions = general characteristics
                        - Time dimension shows WHEN decision is made
                        - Frequency dimension shows WHERE in spectrum
                        """)
                    
                    elif method_key == "shap":
                        st.markdown("""
                        ### SHAP Feature Importance Guide
                        
                        **Red bars**: Increase FAKE probability
                        **Blue bars**: Increase REAL probability
                        
                        **Key features**:
                        - Spectral Centroid/Rolloff: Voice quality
                        - MFCCs: Speech characteristics
                        - Zero Crossing Rate: Signal complexity
                        - Tempo/Rhythm: Speaking patterns
                        """)
